...
detect_intel_iommu函数主要作用就是获取dmar acpi表，然后解析表里面的相关信息如果表里面remapping structure为drhd则通过cb函数来验证dma remapping hardware unit是否可用，具体是dmar_validate_one_dh。就是指定iommu_init函数入口为intel_iommu_init。

ommu_init_mempool：为iova, iommu_dmoain, devinfo创建内存池。
